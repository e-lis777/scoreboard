...
function allowedPath(parts) {
  const value = parts.join('/');
  return /^(?:teams\/\d+(?:\/schedule)?|games\/\d+)$/.test(value) ? value : null;
}

export default async function handler(request, response) {
  if (request.method !== 'GET') return response.status(405).json({ error: 'Method not allowed' });
  const parts = Array.isArray(request.query.parts) ? request.query.parts : [request.query.parts].filter(Boolean);
  const path = allowedPath(parts);
